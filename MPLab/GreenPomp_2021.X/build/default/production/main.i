...
unsigned int voltage = (unsigned int)(50U * ADC_GetConversion(ADC) / 1024U) ;
if ( voltage >= 42 ) {
return (1);
} else if (voltage >= 39) {
return (2);
...
void TimeCheck_ISR() {
waitTimeCounter++;
if( waitTimeCounter >= target_count ){
waitTimeCounter=0;
fireFlg=1;
}
}
...
unsigned uint16_t delta = (0xFFFF-T0ReloadVal1) >> 3 ;
unsigned uint16_t timer0ReloadVal16bit2 = 2 * delta + T0ReloadVal1;
unsigned uint16_t timer0ReloadVal16bit3 = 3 * delta + T0ReloadVal1;
...
TMR0_WriteTimer(timer0ReloadVal16bit3);
TMR0_StartTimer();
TM1640_dispINT(sec);
...
if (PORTAbits.RA2 + PORTAbits.RA2 + PORTAbits.RA2==0) {
...
if (PORTAbits.RA1 + PORTAbits.RA1 + PORTAbits.RA1==0){
if (index < 5) {
index++;
...
TM1640_dispINT(delay_counts);
TMR0_WriteTimer(timer0ReloadVal16bit2);
while (1) {
if(PIR0bits.TMR0IF){
PIR0bits.TMR0IF=0;
TMR0_WriteTimer(timer0ReloadVal16bit3);
...
if(PORTAbits.RA1+PORTAbits.RA1+PORTAbits.RA1==3){
TMR0_StopTimer();
goto BEGIN;
}
}
...
adc_integration=0;
PIR0bits.TMR0IF=1;
while(adc_integration < target_integration){
if (PIR0bits.TMR0IF){
adcCounts = ADC_GetConversion(ADC);
adc_integration = adc_integration + adcCounts;
TM1640_dispVoltage(adcCounts);
